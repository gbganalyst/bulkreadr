...
#' @description
#' Import data from multiple sheets in a Google Sheets spreadsheet and then append dataframe from each sheet into a single dataframe.
#'
#' @inheritParams dplyr::bind_rows
#' @inheritParams googlesheets4::read_sheet
#' @return
#' A [tibble][tibble::tibble()]. If there are parsing problems, a warning will alert you.
#' @seealso
#' [googlesheets4::read_sheet()] which reads a Google (spread)Sheet into a data frame.
#'
#' @importFrom googlesheets4
#'
#' @export
#'
#' @examples
...
read_gsheets <- function(ss, col_types = NULL, .id = NULL) {
  ss %>%
    sheet_names() %>%
    set_names() %>%
    map_df(read_sheet, ss = ss, col_types = col_types, .id = .id)
}
